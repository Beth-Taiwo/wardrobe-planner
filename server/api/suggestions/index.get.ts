...
export default defineEventHandler((event) => {
  ...
  const windowDays = typeof query.windowDays === "string" ? Math.max(30, Math.min(90, Number(query.windowDays) || 60)) : 60

  if (!datePattern.test(date)) {
    throw createError({
      statusCode: 400,
      statusMessage: "Provide a target date as YYYY-MM-DD."
    })
  }
  ...
  const suggestions = [...latestByTitle.values()]
    .filter((candidate) => !recentTitles.has(normalizeTitle(candidate.row.title)))
    .map((candidate) => {
      ...
      const score =
        ...
        (sameWeekday ? 24 : 0) +
        (workdayFit ? 16 : 0) +
        Math.min(16, candidate.count * 2) +
        Math.min(16, Math.floor(daysSinceWorn / 30))
      const reasons = [
        "Not worn within " + windowDays + " days of " + date,
        "Last worn on " + candidate.row.date
      ]

      if (sameMonth) {
        reasons.push("Has worked well in April before")
      }

      if (sameWeekday) {
        reasons.push("Matches the same weekday pattern")
      }

      if (workdayFit) {
        reasons.push("Fits a workday category")
      }

      return {
        entry: toDressEntry(candidate.row),
        score,
        lastWornDate: candidate.row.date,
        daysSinceWorn,
        reasons
      }
    })
